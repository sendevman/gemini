...
CREATE OR REPLACE VIEW VW_SIE_NYE_STUDENT_ENROLLMENT AS
  ...
  FROM PRDE_TMAXIO1.ST_ENROLLMENT E
    LEFT JOIN PRDE_TMAXIO1.ENUM_ST_END_STATUS_CODE N ON E.END_STATUS_CD = N.VALUE AND E.SCHOOL_YEAR = N.SCHOOL_YEAR
    LEFT JOIN PRDE_TMAXIO1.ENUM_ST_SPECIAL_PRG_CATEGORY O ON E.SPECIAL_PROG_CATEGORY_CD = O.VALUE
    LEFT JOIN PRDE_TMAXIO1.ENUM_ST_ENTRY_CODE P ON E.ENTRY_CD = P.VALUE;
...
CREATE OR REPLACE VIEW VW_VOCATIONAL_SCHOOLS AS
  ...
  FROM PRDE_TMAXIO1.SY_SCHOOL S
    ...
    LEFT JOIN PRDE_TMAXIO1.ENUM_CE_COUNTY_CODE CC ON S.COUNTY_CD = CC.VALUE AND CC.IS_ACTIVE_IND = 1
    LEFT JOIN PRDE_TMAXIO1.SY_FLEX_DATA FD ON S.SCHOOL_ID = FD.TYPE_ID AND FD.FLEX_OWNER_ID = 604
  WHERE S.IS_ACTIVE_IND = 1 AND FD.FIELD_36 IS NOT NULL
...
CREATE OR REPLACE VIEW VW_VOCATIONAL_PROGRAMS AS
  SELECT DISTINCT
    ...
    P.DESCRIPTION ,
    C.LONG_DESCRIPTION
  FROM PRDE_TMAXIO1.VW_SIE_VOC_SCHOOL_COURSES C
    LEFT JOIN PRDE_TMAXIO1.ENUM_ST_CTE_COMPLTR_PROG_CD P ON SUBSTR(C.EXT_COURSE_NUMBER, 0, 4) = P.VALUE
  WHERE SUBSTR(EXT_COURSE_NUMBER, 6, 3) = '133'
        AND P.VALUE IS NOT NULL
  ORDER BY P.VALUE, P.DESCRIPTION;
